...
variable "domain_name" {
  description = "Base domain name for the environment (e.g., smoothbill.fr)"
  type        = string
...
variable "ssh_key_name" {
  ...
  type        = string
  default     = "izzzi-prod-key"
}

variable "allowed_ssh_ips" {
  description = "List of IP addresses/CIDR blocks allowed to access SSH. REQUIRED in production - must not be empty or contain 0.0.0.0/0"
  type        = list(string)
  default     = []
  
  validation {
    condition = length(var.allowed_ssh_ips) > 0 && !contains(var.allowed_ssh_ips, "0.0.0.0/0")
    error_message = "allowed_ssh_ips must not be empty and must not contain 0.0.0.0/0 in production environment for security reasons"
  }
}
...
variable "database_user" {
  description = "PostgreSQL database user (for connection string template in outputs)"
  type        = string
  sensitive   = true
  default     = "izzzi_user"
}
...
variable "database_password" {
  description = "PostgreSQL database password (for connection string template in outputs)"
  type        = string
  sensitive   = true
  default     = ""
}
